...
export const useTransfer = () => {
  ...
  const sendTransfer = async (props: {
    ...
    const outSenderNote = BarretenbergService.generateNote(
      // TODO: ,
      outSenderAmount
    );

    const outReceiverNote = BarretenbergService.generateNote(
      props.to.address,
      props.amount
    );
    ...
    const orderedNotes = notes.sort((a, b) => parseInt((b.index! - a.index!).toString()));
    ...
    try {
      const generatedProof = await ProofService.generateProof({
        ...
        in_commitment_nullifier_hash: BarretenbergService.generateHashArray([new Fr(inputNote.nullifier!)]).toString(16),
        ...
        out_receiver_account: props.to.address,
